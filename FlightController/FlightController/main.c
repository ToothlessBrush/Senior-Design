...
{
    dshotHigh, dshotLow, dshotLow, dshotLow, 
    dshotLow, dshotLow, dshotHigh, dshotLow, 
    dshotHigh, dshotHigh, dshotLow, dshotLow, 
...
void DMAInit()
{
    ...
    DMA1_Stream5->PAR = (uint32_t) &TIM2->CCR1; // Set the peripheral memory address to 
    DMA1_Stream5->M0AR = (uint32_t) dshotBuffer; // Set the memory location to the buffer
    DMA1_Stream5->FCR = 0;
...
void DMA1_Stream5_IRQHandler()
{
    ...
    for(volatile int i = 0; i < 5; i++); // forced delay to wait for final bit
    ...
    for (volatile int i = 0; i < 16; i++); // Forced delay between dshot frames
    DMA1_Stream5->CR |= 1;
    // TIM2->CCMR1 |= (6<<4); // Set to PWM mode 1
    TIM2->CR1 |= 1;
}
