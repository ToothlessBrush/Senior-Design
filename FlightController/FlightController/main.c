...
int main(void) {
    // If you need to set SystemCoreClock ensure it's 84 MHz before using PSC formula above.
    // SystemInit() from startup file typically sets clocks; ensure it's called.
    ...
    while (1) {
        ...
        else if(((GPIOA->IDR) & 0x6) == 0x6)
        {
            ...
            {
                TIM2->CCR1 = i;
                for(volatile int j = 0; j < 10000; j++) __NOP();
            }
        }
        else 
        {
