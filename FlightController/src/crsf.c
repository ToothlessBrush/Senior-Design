...
static volatile bool crsfFrameDone = false;
...
// buffer. uart_read_byte() should return -1 (or false via the bool out-param)
// when empty.
void crsf_process(void) {

    static uint8_t pos = 0;
    static uint32_t frameStartMs = 0; // optional gap reset — wire to systick

    int16_t c;
    while (uart_data_available(UART_INSTANCE_1)) {
        c = uart_receive_byte(UART_INSTANCE_1);
        uint8_t byte = (uint8_t)c;

        // Determine expected full frame length once we have byte 1
        // (frameLength) Full frame = address(1) + frameLength_field(1) +
        // frameLength bytes frameLength itself includes type + payload + CRC
        // but NOT address/len fields.
        const int fullFrameLength = (pos < 3)
                                        ? 5
        ...
        if (pos < clampedLength) {
            crsfFrame.bytes[pos++] = byte;
        }
        ...
        if (pos >= clampedLength) {
            ...
            if (crc == crsfFrame.bytes[clampedLength - 1]) {
                ...
                case CRSF_FRAMETYPE_SUBSET_RC_CHANNELS_PACKED:
                    if (crsfFrame.frame.deviceAddress ==
                        ...
                        // Copy before setting flag — avoid race if you ever
                        // move crsf_process into an IRQ context
            ...
            pos = 0; // reset regardless of CRC outcome
        }
    }
