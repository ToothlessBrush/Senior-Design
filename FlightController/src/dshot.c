...
void InitDshot(dshotMotor *motor1, dshotMotor *motor2, dshotMotor *motor3,
    ...
    //               B0      B1      B4        B5
    GPIOB->MODER |= 2 | (2 << 2) | (2 << 8) |
                    (2 << 10); // Set Pin 0-3 to alternate function mode'
    GPIOB->PUPDR |= 2 | (2 << 2) | (2 << 8) | (2 << 10);
    ; // Enable pull-down resistor
    GPIOB->OSPEEDR |= 3 | (3 << 2) | (3 << 8) | (3 << 10);
    ;                           // High output speed
    GPIOB->AFR[0] &= ~0xFF00FF; // Clear function
    GPIOB->AFR[0] |= (2 | (2 << 4) | (2 << 16) |
                      (2 << 20)); // Set alternate function to AF02, TIM3
    ...
    TIM3->CCER &=
        ~(2 | (2 << 4) | (2 << 8) | (2 << 12)); // Active high on output
    TIM3->CCER |= (1 | (1 << 4) | (1 << 8) |
                   (1 << 12)); // Output to the pin B0, B1, B4, B5
    ...
    DMA1_Stream4->PAR =
        (uint32_t)&TIM3->CCR1; // Set the peripheral memory address to
    DMA1_Stream4->M0AR =
        (uint32_t)motor1->dshotBuffer; // Set the memory location to the buffer
    DMA1_Stream4->FCR = 0;
    ...
    DMA1_Stream5->PAR =
        (uint32_t)&TIM3->CCR2; // Set the peripheral memory address to
    DMA1_Stream5->M0AR =
        (uint32_t)motor2->dshotBuffer; // Set the memory location to the buffer
    DMA1_Stream5->FCR = 0;
    ...
    DMA1_Stream7->PAR =
        (uint32_t)&TIM3->CCR3; // Set the peripheral memory address to
    DMA1_Stream7->M0AR =
        (uint32_t)motor3->dshotBuffer; // Set the memory location to the buffer
    DMA1_Stream7->FCR = 0;
    ...
    DMA1_Stream2->PAR =
        (uint32_t)&TIM3->CCR4; // Set the peripheral memory address to
    DMA1_Stream2->M0AR =
        (uint32_t)motor4->dshotBuffer; // Set the memory location to the buffer
    DMA1_Stream2->FCR = 0;
