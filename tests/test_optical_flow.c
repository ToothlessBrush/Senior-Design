 ...
 * telemetry via Bluetooth when data is received.
 */
...
int main(void) {
    ...
    // Initialize Bluetooth on UART2
    comm_init();

    delay_ms(100);
    comm_send_string("LOG:OPTICAL_FLOW_TEST_START");

    delay_ms(100);
    comm_send_string("LOG:INITIALIZING_OPTICAL_FLOW");

    // Initialize optical flow sensor on UART6
    optical_flow_init();

    delay_ms(100);
    comm_send_string("LOG:OPTICAL_FLOW_READY");
    comm_send_string("LOG:RAW_HEX_DUMP_MODE");
